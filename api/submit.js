...
app.post('/submit', upload.fields([{ name: 'fotos' }, { name: 'videos' }]), async (req, res) => {
  try {
    ...
    // Quando o PDF terminar de gerar
    pdfDoc.on('end', async () => {
      ...
      archive.on('end', async () => {
        ...
        try {
          ...
          return res.status(500).json({ error: 'Erro ao enviar e-mail. Verifique usuário e senha do SMTP.' });
        }
      });
    });
    ...
    return res.status(500).json({ error: 'Erro ao gerar e enviar relatório' });
  }
});

// Exporta para serverless (Vercel)
